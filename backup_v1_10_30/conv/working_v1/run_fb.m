...
for i_s = 1:length(SNRdB_vec)
    ...
    for i_on = 1:nOut
        ...
        parfor i_n = 1:nMiniFrames
            ...
            [dataIn, rr_len] = conv_enc(data, targetCodeRate, 1);
            ...
            if (comm_mod == 1)
                ...
                if (mod_approx == 0)
                    noiseVar = 1./(10.^(SNRdB/10));
                    rxLLR = lteSymbolDemodulate(rxSig,modulation,noiseVar);
                else
                    rxLLR = 1 - 2*double(lteSymbolDemodulate(rxSig,modulation,'DecisionType','hard'));
                end
            end

            % Rate recovery and Decoding
            [data_est, crc_chk] = conv_dec(rxLLR, rr_len, 1);
...
semilogy(SNRdB_vec,BER_vec_FB); hold on;
