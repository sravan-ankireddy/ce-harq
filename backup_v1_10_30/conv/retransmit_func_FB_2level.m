    ...
    for i_r = 1:max_rounds-1
        ...
        if (fb_scheme == "FB")
            ...
            if (inner_scheme == "FB")
                ...
                % find the smallest rate for compression to map to K bits
                ...
                % Interleave
                % errDataInSeq = randintrlv(errDataInSeq,0);

                % now perform outer coding with original baserate
                [errDataInSeq, rr_len_err] = conv_enc(errDataInSeq, R);
            ...
            outer_err_seq_est = conv_dec(rxLLR_FB, R);
            
            outer_err_seq_est = int16(outer_err_seq_est);
