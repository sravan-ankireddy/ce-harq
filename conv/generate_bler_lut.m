...
for i_e = 1:gs_size
	...
	hold on;
	leg_str{end+1} = sprintf('Code Rate %.3f',code_rates(i_e));
end
xlabel('SNR');
ylabel('BLER');
legend(leg_str);
...
% Store data 
data_file_name_gs = [res_folder_fb sprintf('/harq_data_Conv_%d_rate_%.3f_rate_%.3f_max_rounds_%d.mat', N, code_rates(1),code_rates(end), max_rounds)];
save(data_file_name_gs,'ber_data','bler_data','ar_data','snr_data','code_rates');
