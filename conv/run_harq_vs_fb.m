...
common_str = [res_folder_harq_vs_fb sprintf('/BLER_HARQ_vs_FB_Conv_%d_rate_%.3f_err_thr_%.3f_max_rounds_%d', N,R, err_thr, max_rounds)];
...
title_name = sprintf('FB-%s : Avg rounds Conv %d, mod. %s, Rate %.3f, errthr %.3f, max. rounds %d',combining_scheme, N, modulation, R, err_thr, max_rounds);
title(title_name,'FontSize',fs);

common_str = [res_folder_harq_vs_fb sprintf('/Avg_rounds_HARQ_vs_FB_Conv_%d_rate_%.3f_err_thr_%.3f_max_rounds_%d', N,R, err_thr, max_rounds)];
