...
SNRdB_vec = -10:0.2:20; % SNR in dB
BLER = zeros(size(SNRdB_vec));
codeRate = 0.8;
...
parfor i = 1:length(SNRdB_vec)
    ...
    NRB = 4;                         % BW in number of RBs (52 RBs at 15 kHz SCS for 10 MHz BW)
    ...
    BLER(i) = numBlkErr/noTransportBlocks;
...
f = semilogy(SNRdB_vec,BLER_IR,'-d');
hold on;
semilogy(SNRdB_vec,BLER_Chase,'-d');
xlabel('SNR');
ylabel('BLER');

max_rounds = length(rvSeq);

leg_IR = sprintf('HARQ-IR max. %d rounds',max_rounds);
leg_Chase = sprintf('HARQ-Chase max. %d rounds',max_rounds);
legend(leg_IR,leg_Chase);
trBlkSizes = 1032;

fig_name = sprintf('ref_curves/IRvsChase_BLER_LDPC_%d_HARQ_%d.fig',trBlkSizes,max_rounds);
savefig(fig_name);
png_name = sprintf('ref_curves/IRvsChase_BLER_LDPC_%d_HARQ_%d.png',trBlkSizes,max_rounds);
saveas(f,png_name);